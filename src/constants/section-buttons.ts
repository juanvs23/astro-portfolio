export interface SectionButtonDef {
  /** i18n key used for the button label (e.g., 'menu.about') */
  key: string;
  /** URL path segment (appended to /{locale}/) */
  path: string;
  /** SVG icon markup (inline SVG string) */
  icon: string;
  /** Position key for orbital layout around central 3D object */
  position: string;
}

/**
 * Returns the ordered list of section navigation button definitions.
 *
 * The order matches the nav bar: About → Skills → Services → Works → Projects → Contact.
 * Each entry provides the i18n key, URL path segment, and SVG icon for rendering.
 */
export function getSectionButtonDefs(): SectionButtonDef[] {
  return [
    {
      key: 'menu.about',
      path: 'about',
