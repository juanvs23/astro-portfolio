...
The event names (`whatsapp_cta`, `plan_whatsapp_cta`, etc.) are **not** written on the buttons.
They are generated by the site code when a click happens. There is **one global click listener**
in `src/lib/gtm-client.ts` that checks every click like this:

```
click → is it an <a href*="wa.me">?  ── no  → ignore
                  │ yes
                  ├─ has data-gtm-plan?        → push { event: 'plan_whatsapp_cta', plan }
                  ├─ inside a form (LeadForm)? → ignore (the form fires its own event)
                  └─ otherwise                 → push { event: 'whatsapp_cta' }
```

So a **general** WhatsApp button needs **no marker** — the site detects it by its `wa.me` link.
Only the **3 plan** CTAs carry a `data-gtm-plan` attribute so the site can tell which plan was clicked.

The **trigger** you create in GTM (Step 4) is a **Custom Event** that listens for these exact names
in the `dataLayer`. The code does the detection; GTM does the matching.
